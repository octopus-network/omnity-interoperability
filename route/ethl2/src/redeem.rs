...
use ethers_core::{abi::RawLog, types::{Address, H32, U256}};
use evm_rpc::{candid_types::{self, BlockTag}, MultiRpcResult, RpcServices};
use crate::*;
    ...
    match rpc_result {
        MultiRpcResult::Consistent(result) => {
            for log_entry in result.expect("Evm rpc error") {
                let raw_log = RawLog {
                    topics: log_entry.topics.iter().map(|topic| topic.0.into()).collect_vec(),
                    data: log_entry.data.0.clone(),
                };
                let token_burned = TokenBurned::decode_raw_log(
                    vec![
                       WordToken(TokenBurned::SIGNATURE_HASH) 
                    ], 
                    &raw_log.data, 
                    false
                ...
                ic_cdk::call(
                    super::hub_addr().ok_or(super::Error::RouteNotInitialized)?,
                     "send_ticket", 
                     (ticket,)
                    )
                .await
