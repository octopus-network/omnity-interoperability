...
pub const TO_TON_TICKETS_MEMORY_ID: MemoryId = MemoryId::new(1);
pub const TO_TON_DIRECTIVES_MEMORY_ID: MemoryId = MemoryId::new(2);
...
pub const FINALIZED_MINT_REQUESTS_MEMORY_ID: MemoryId = MemoryId::new(6);
...
pub fn get_to_ton_tickets_memory() -> Memory {
    with_memory_manager(|m| m.get(TO_TON_TICKETS_MEMORY_ID))
}

pub fn get_to_ton_directives_memory() -> Memory {
    with_memory_manager(|m| m.get(TO_TON_DIRECTIVES_MEMORY_ID))
}
...
pub fn get_finalized_mint_requests_memory() -> Memory {
    with_memory_manager(|m| m.get(FINALIZED_MINT_REQUESTS_MEMORY_ID))
}

pub fn get_upgrade_stash_memory() -> Memory {
    with_memory_manager(|m| m.get(UPGRADE_STASH_MEMORY_ID))
}
pub fn init_to_ton_tickets_queue() -> StableBTreeMap<u64, Ticket, Memory> {
    StableBTreeMap::init(get_to_ton_tickets_memory())
}

pub fn init_pending_ticket_map() -> StableBTreeMap<Seq, PendingTicketStatus, Memory> {
...
pub fn init_to_ton_directives_queue() -> StableBTreeMap<u64, Directive, Memory> {
    StableBTreeMap::init(get_to_ton_directives_memory())
}
...
pub fn init_finalized_mint_requests() -> StableBTreeMap<TicketId, String, Memory> {
    StableBTreeMap::init(with_memory_manager(|m| m.get(FINALIZED_MINT_REQUESTS_MEMORY_ID)))
}
