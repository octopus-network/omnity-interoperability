...
use crate::base::{Error, get_time_secs};
use crate::base::const_args::{BATCH_QUERY_LIMIT, FETCH_HUB_DIRECTIVE_INTERVAL, FETCH_HUB_TICKET_INTERVAL, MONITOR_PRINCIPAL, SCAN_EVM_TASK_INTERVAL, SEND_EVM_TASK_INTERVAL};
use crate::hub_to_route::{fetch_hub_directive_task, fetch_hub_ticket_task};
use crate::route_to_ton::{send_directive, send_ticket, to_evm_task};
use crate::stable_log::{init_log, StableLogWriter};
use crate::stable_memory::init_stable_log;
use crate::state::{
    TonRouteState, init_chain_pubkey, minter_addr, mutate_state, read_state, replace_state,
    StateProfile,
};
use omnity_types::{
...
async fn generate_ticket(hash: String) -> Result<(), String> {
    ...
    //TODO let (ticket, _transaction_receipt) = create_ticket_by_tx(&tx_hash).await?;
