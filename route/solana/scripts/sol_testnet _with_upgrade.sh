...
echo "testnet environment: 
    ...
    schnoor canister id: $SCHNORR_CANISTER_ID
...
# install or reinstall solana route
...
dfx canister install $SOL_PROVIDER_CANISTER_ID --argument "( record { 
    ...
    --mode=reinstall -y \
    --wasm=./assets/ic_solana_provider20240918.wasm.gz \
    --ic 
...
dfx canister call $SOL_PROVIDER_CANISTER_ID sol_latestBlockhash "(opt \"${alchemy}\")" --ic
dfx canister call $SOL_PROVIDER_CANISTER_ID sol_getAccountInfo "(\"${test_account}\",opt \"${alchemy}\")" --ic
dfx canister call $SOL_PROVIDER_CANISTER_ID sol_getSignatureStatuses "(vec {\"${test_sig}\"},opt \"${alchemy}\")" --ic
echo 
...
dfx canister install $SOLANA_ROUTE_CANISTER_ID --argument "(variant { Init = record { \
    ...
    --mode=reinstall -y \
    --wasm=./assets/solana_route20240918.wasm.gz \
    --ic 
...
dfx canister call $SOLANA_ROUTE_CANISTER_ID get_transaction "(\"${test_sig}\",opt \"${alchemy}\")" --ic
# update schnorr info
...
# add token
TOKEN_ID="Bitcoin-runes-HOPE•YOU•GET•NICE202409242137"
TOKEN_NAME="HOPE•YOU•GET•NICE202409242137"
TOKEN_SYMBOL="NICE202409242137"
DECIMALS=2
...
# upgrade
echo "upgrade sol-provider-canister: $SOL_PROVIDER_CANISTER_ID ..."
dfx canister install $SOL_PROVIDER_CANISTER_ID --argument "( record { 
...
dfx canister call $SOLANA_ROUTE_CANISTER_ID get_transaction "(\"${SIGNAURE}\",opt \"${alchemy}\")" --ic
...
# uprade test
echo "upgrade omnity hub : $SOL_PROVIDER_CANISTER_ID ..."
