...
use crate::ic_sui::fastcrypto::encoding::{Base58, Encoding};
use crate::ic_sui::sui_types::{error::SuiError, sui_serde::Readable};
...
impl TryFrom<&[u8]> for TransactionDigest {
    type Error = crate::ic_sui::sui_types::error::SuiError;

    fn try_from(bytes: &[u8]) -> Result<Self, crate::ic_sui::sui_types::error::SuiError> {
        let arr: [u8; 32] = bytes
            .try_into()
            .map_err(|_| crate::ic_sui::sui_types::error::SuiError::InvalidTransactionDigest)?;
        Ok(Self::new(arr))
    }
}
