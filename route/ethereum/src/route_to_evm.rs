...
use crate::eth_common::{broadcast, call_rpc_with_retry, get_account_nonce, get_gasprice};
...
use ethereum_common::contracts::{gen_evm_tx, gen_execute_directive_data, gen_mint_token_data};
use ethereum_common::error::Error;
use ethereum_common::error::Error::Custom;
use ethereum_common::signs::sign_transaction;
        ...
        match ret {
            ...
            Err(e) => match e {
                ...
                _ => {
                    log!(
                        WARNING,
                        "[evm_route] send directive to evm error: {}",
                        e.to_string()
                    );
                }
            },
    ...
    match read_state(|s| s.tickets_queue.get(&seq)) {
        ...
        Some(t) => {
            ...
            let nonce = call_rpc_with_retry(minter_addr(), get_account_nonce)
                .await
                .unwrap_or_default();
            let tx = gen_evm_tx::<EthereumStateProvider>(
                data_result,
            ...
            match sign_transaction::<EthereumStateProvider>(tx).await {
                Ok(data) => {
                    let hash = call_rpc_with_retry(data.clone(), broadcast).await;
                    match hash {
                        Ok(_h) => {
                            let tx_hash = keccak256(data).encode_hex_with_prefix();
...
pub async fn send_directive(seq: Seq) -> Result<Option<String>, Error> {
    match read_state(|s| s.directives_queue.get(&seq)) {
        None => Ok(None),
        Some(d) => {
            let data = gen_execute_directive_data::<EthereumStateProvider>(&d, U256::from(seq));
            if data.is_empty() {
