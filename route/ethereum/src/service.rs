...
use crate::const_args::{
    BATCH_QUERY_LIMIT, LIGHTCLIENT_CHECK_INVERVAL, LIGHTCLIENT_CHECK_TASK_NAME, MONITOR_PRINCIPAL,
    PERIODIC_TASK_INTERVAL, SCAN_EVM_TASK_NAME, SEND_EVM_TASK_NAME,
};
use crate::eth_common::{call_rpc_with_retry, get_balance};
...
use crate::lightclient::verify_task;
use crate::route_to_evm::{
    send_directive, send_directives_to_evm, send_ticket, send_tickets_to_evm,
};
use crate::state::{
    get_redeem_fee, init_chain_pubkey, minter_addr, mutate_state, read_state, replace_state,
    EvmRouteState, StateProfile,
};
...
use omnity_types::{hub, Network};
use omnity_types::{Chain, ChainId, Directive, MintTokenStatus, Seq, Ticket, TicketId};
...
fn post_upgrade() {
    ...
    log!(
        INFO,
        "[evmroute] upgraded successed at {}",
        ic_cdk::api::time()
    );
}
...
fn start_tasks() {
    set_timer_interval(
        Duration::from_secs(PERIODIC_TASK_INTERVAL),
        bridge_ticket_to_evm_task,
    );
    set_timer_interval(Duration::from_secs(PERIODIC_TASK_INTERVAL), scan_evm_task);
    set_timer_interval(
        Duration::from_secs(LIGHTCLIENT_CHECK_INVERVAL),
        lightclient_check_task,
    );
}
...
pub fn lightclient_check_task() {
    ic_cdk::spawn(async {
        let _guard =
            match crate::guard::TimerLogicGuard::new(LIGHTCLIENT_CHECK_TASK_NAME.to_string()) {
                Some(guard) => guard,
                None => return,
            };
        verify_task().await;
...
fn set_port_address(port_addr: String) {
    ...
    log!(
        INFO,
        "CONTROLLER_OPERATION: {}, update port address PARAMS: {}",
        user.to_text(),
        port_addr.as_str()
    );
...
#[update(guard = "is_admin")]
async fn query_tx(tx: String) -> Option<Result<(), String>> {
    let h = read_state(|s| s.lightclient_verify_result.get(&tx).cloned());
    h
}
...
fn update_rpc_check_rate(min_resp_count: usize, total_required_rpc_count: usize) {
    assert!(
        min_resp_count > 0 && total_required_rpc_count >= min_resp_count,
        "params errorr"
    );
    let rpc_size = read_state(|s| s.rpc_providers.len());
    assert!(
        rpc_size >= total_required_rpc_count,
        "rpc count is not enough"
    );
    mutate_state(|s| {
...
async fn generate_ticket(hash: String) -> Result<(), String> {
    log!(
        INFO,
        "[generate ticket] received generate ticket request: {}",
        &hash
    );
