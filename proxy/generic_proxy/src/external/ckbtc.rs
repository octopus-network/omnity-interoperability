...
use icrc_ledger_types::{
    icrc1::{account::{Account, Subaccount}, transfer::BlockIndex},
...
pub async fn approve_ckbtc_for_icp_custom(
    ...
    let ckbtc_ledger_principal = state.ckbtc_ledger_principal.clone();
    let client = ICRC1Client {
        runtime: CdkRuntime,
        ledger_canister_id: ckbtc_ledger_principal,
    };
    let spender = Account {
        owner: state.icp_customs_principal,
        subaccount: None,
    };
...
pub async fn get_ckbtc_transaction(block_index: BlockIndex) -> Result<GetTransactionsResponse> {
    let ckbtc_ledger_principal = state::get_state().ckbtc_ledger_principal;
...
pub async fn update_balance(args: UpdateBalanceArgs)-> Result<Vec<UtxoStatus>> {
    ...
    ic_cdk::api::call::call(state.ckbtc_minter_principal, "update_balance", (args.clone(),))
        .await
        .map_err(|(code, msg)| {
            Errors::CanisterCallError(
                state.ckbtc_minter_principal.to_string(),
                "get_transactions".to_string(),
                format!("{:?}", code),
                msg,
            )
...
pub async fn get_btc_address(args: GetBtcAddressArgs) -> Result<String> {
    let ckbtc_minter_principal = state::get_state().ckbtc_minter_principal;
