...
use crate::types::{Destination, LockTicketRequest, ReleaseTokenStatus, RpcConfig, TokenResp};
...
fn init(args: InitArgs) {
    ...
    mutate_state(|s| {
        s.default_rpc_config = RpcConfig {
            url: "https://doge-mainnet.gateway.tatum.io".to_string(),
            api_key: Some("t-66e18234402072e5439fc774-8febf4c3594f4d25a9af3b98".to_string()),
        }
    });
    start_tasks();
}
...
#[update]
pub async fn generate_ticket(req: GenerateTicketArgs) -> Result<(), CustomsError> {
...
#[update(guard = "is_admin")]
pub async fn set_rpc_config(url: String, api_key: Option<String>) {
    mutate_state(|s| {
        s.default_rpc_config = RpcConfig { url, api_key };
    });
}
...
pub async fn test_http(
    ...
    let doge_rpc = DogeRpc { url, api_key };
    doge_rpc
        .get_raw_transaction(txid.as_str())
        .await
        .map(|r| format!("{:?}", r))
}

#[update(guard = "is_admin")]
pub async fn tmp_fix() {
    mutate_state(|s| {
        s.pending_lock_ticket_requests.clear();
    });
}
...
//     check_tx_confirmation(txid).await;
//     finalize_ticket(txid).await;
// }
...
pub async fn resend_unlock_ticket(seq: Seq, fee_rate: Option<u64>) -> Result<String, String> {
    match crate::custom_to_bitcoin::submit_unlock_ticket(seq, fee_rate).await {
        Ok(r) => {
            let r = r.unwrap();
...
fn query_finalized_lock_tickets(txid: String) -> Option<LockTicketRequest> {
    let txid = Txid::from_str(txid.as_str()).unwrap();
    read_state(|s| s.finalized_lock_ticket_requests.get(&txid.into()).cloned())
}
