...
use ic_stable_structures::{log::{Log as StableLog, NoSuchEntry}, memory_manager::{MemoryId, MemoryManager, VirtualMemory}, DefaultMemoryImpl, StableBTreeMap, StableVec};
...
pub fn init_pending_etching_requests() -> StableBTreeMap<String, SendEtchingRequest, VMem> {
    StableBTreeMap::init(with_memory_manager(|m|m.get(PENDING_ETCHING_REQUESTS_MEMORY_ID)))
}

pub fn init_finalized_etching_requests() -> StableBTreeMap<String, SendEtchingRequest, VMem> {
    StableBTreeMap::init(with_memory_manager(|m|m.get(FINALIZED_ETCHING_REQUESTS_MEMORY_ID)))
}
