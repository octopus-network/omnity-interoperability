...
pub async fn etching_rune(fee_rate: u64, args: &InternalEtchingArgs) -> anyhow::Result<(SendEtchingRequest, u64)>{
        ...
        .map_err(|e| {
            mutate_state(|s|
                for in_utxo in vins.clone() {
                    s.etching_fee_utxos.push(&in_utxo).expect("retire utxo failed");
                }
            );
            e
        })?;
    ...
    if send_res.status == SendCommitSuccess {
        ...
        mutate_state(|s|
            for in_utxo in vins {
                s.etching_fee_utxos.push(&in_utxo).expect("retire utxo failed1");
            }
        );
    }
...
pub async fn generate_etching_transactions(fees: Fees, vins: Vec<Utxo>,args: &InternalEtchingArgs) -> anyhow::Result<BuildEtchingTxsResult> {
    ...
    let sender = Address::from_str(etching_account.address.as_str()).unwrap().assume_checked();
    ...
    let mut inscription = Nft::new(
        ...
        Some(etching.rune.clone().unwrap().to_string().as_bytes().to_vec()),
        args.logo.clone()
    );
    ...
    let receipient = Address::from_str(receipient.as_str()).unwrap().assume_checked();
    let reveal_transaction = builder
        ...
        .await?;
    Ok(BuildEtchingTxsResult{txs: vec![signed_commit_tx, reveal_transaction],script_out_address: commit_tx.script_out_address})
}
...
pub async fn estimate_tx_vbytes(rune_name: &str, logo: Option<LogoParams>) -> OrdResult<(usize, usize)>{
    let fees = Fees {
        commit_fee: Amount::from_sat(1000),
        reveal_fee: Amount::from_sat(20000),
    };
    let sender = Address::from_str("bc1qyelgkxpfhfjrg6hg8hlr9t4dzn7n88eajxfy5c").unwrap().assume_checked();
    let vins = vec![Utxo {
    ...
    inscription.rune = Some(
        etching
            .rune
            .ok_or(anyhow::anyhow!("Invalid etching data; rune is missing")).unwrap()
            .commitment(),
    );
    let mut builder = OrdTransactionBuilder::p2tr(
        PublicKey::from_str("02eec672e95d002ac6d1e8ba97a2faa9d94c6162e2f20988984106ba6265020453").unwrap(), //TODO
        sender.clone(),
    );
    let commit_tx = builder
        .estimate_commit_transaction(
            CreateCommitTransactionArgsV2 {
                inputs: vins.clone(),
                inscription,
                txin_script_pubkey: sender.script_pubkey(),
