...
use crate::runes_etching::{constants, Inscription, InscriptionParseError, LogoParams, OrdError, OrdResult};
...
impl Nft {
    ...
    pub fn new(content_type: Option<Vec<u8>>, body: Option<Vec<u8>>, logo: Option<LogoParams>) -> Self {
        Self {
            content_type,
            body,
            logo,
    ...
    pub fn append_reveal_script_to_builder(
        ...
        pbb.extend_from_slice(self.rune.clone().unwrap().as_slice()).unwrap();
        ...
        if let Some(l) = self.logo.clone() {
            ...
            let hex = base64::engine::general_purpose::STANDARD.decode(l.content_base64).unwrap();
