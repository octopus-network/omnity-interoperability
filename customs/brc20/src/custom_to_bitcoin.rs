...
use crate::custom_to_bitcoin::CustomToBitcoinError::{ArgumentError, BuildTransactionFailed, SignFailed};
use crate::generate_ticket::GenerateTicketError;
use crate::hub;
use crate::hub::update_tx_hash;
use crate::management::get_utxos;
use crate::ord::builder::{CreateCommitTransactionArgsV2, OrdTransactionBuilder, RevealTransactionArgs, SignCommitTransactionArgs, Utxo};
...
pub async fn send_tickets_to_bitcoin() {
    ...
    if from < to {
        ...
        match utxos {
            Ok(r) => {
                let v = r.utxos.into_iter().map(|u| Utxo{
    ...
    for seq in from..to {
        let res = send_ticket(seq).await;
        if res.is_err() {
            log!(CRITICAL,"send ticket to bitcoin failed {}, {}", seq, res.err().unwrap());
            break;
        }else {
            let r = res.ok().unwrap();
            match r {
        ...
        match tx {
            Ok(t) => {
                if t.status.confirmed {
                    ...
                    let (hub_principal, ticket) = read_state(|s|(s.hub_principal, s.tickets_queue.get(&seq).unwrap()));
                    if let Err(err) =  update_tx_hash(hub_principal, ticket.ticket_id, transfer_txid).await {
                        log!(CRITICAL,
...
pub async fn test_send_ticket(t: Ticket) -> Result<SendTicketResult, CustomToBitcoinError> {
  ...
  //  let token = read_state(|s|s.tokens.get(&t.token).cloned().unwrap());
    ...
    let mut builder = OrdTransactionBuilder::p2tr(PublicKey::from_str(deposit_pubkey().as_str()).unwrap(), key_id, deposit_addr());
    ...
    let commit_tx = builder.build_commit_transaction_with_fixed_fees(
        ...
        CreateCommitTransactionArgsV2 {
            inputs: vins.clone(),
            inscription: Brc20::transfer("nvcs", amount, "bridge-in".to_string(), "bitcoin".to_string()),
    ...
    ).await.map_err(|_|BuildTransactionFailed)?;
    let signed_commit_tx = builder
        ...
        .await.map_err(|e|SignFailed(e.to_string()))?;
    ...
    let transfer_trasaction = build_transfer_transfer(&t, &fees,  real_utxo, &builder.signer(),commit_remain_fee).await?;
...
async fn send_ticket(seq: Seq) -> Result<Option<SendTicketResult>, CustomToBitcoinError> {
    ...
    match ticket {
        ...
        Some(t) => {
            ...
            let commit_tx = builder.build_commit_transaction_with_fixed_fees(
                ...
                CreateCommitTransactionArgsV2 {
                    inputs: vins.clone(),
                    inscription: Brc20::transfer(token.name.clone(), amount, "bridge-in".to_string(), "bitcoin".to_string()),
                    txin_script_pubkey: deposit_addr().script_pubkey(),
                    leftovers_recipient: deposit_addr().clone(),
                    commit_fee: fees.commit_fee,
                    reveal_fee: fees.reveal_fee,
            ...
            let signed_commit_tx = builder
                ...
                .await.map_err(|_|SignFailed("fgeg".to_string()))?;
...
pub async fn build_transfer_transfer(ticket: &Ticket, fee: &Fees, reveal_utxo: Utxo, signer: &MixSigner, commit_return_fee: Option<Utxo>) -> Result<Transaction, CustomToBitcoinError> {
    ...
    let recipient = Address::from_str(&ticket.receiver.to_string()).map_err(|e | ArgumentError(e.to_string()))?.assume_checked();
    let transfer = spend_utxo_transaction(
        ...
        all_inputs,
        fee.utxo_fee,
    ).await.map_err(|_| SignFailed("sgk".to_string()))?;
    Ok(transfer)
}
...
pub fn determine_transfer_fee_txins(fee: &Fees, commit_remain_fee: Option<Utxo>) -> CustomToBitcoinResult<Vec<Utxo>> {
    let fee_amount = fee.utxo_fee.to_sat();
    match commit_remain_fee {
        None => {
            select_utxos(fee_amount)
        }
        Some(t) => {
...
pub fn select_utxos(fee: u64) -> CustomToBitcoinResult<Vec<Utxo>>  {
   /* let mut selected_utxos: Vec<Utxo> = vec![];
    ...
    //TODO
     Ok(vec![Utxo{
         id: Txid::from_str("cce6b27788b89ff1f988a2bcf17ce021f41d9ed54a8cb5bc143ef8b954375bb2").unwrap(),
         index: 0,
